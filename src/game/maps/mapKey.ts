...
const mapKey = function (game: GameType) {
  ...
  const setMapCellPrototypes = (
    mapKey: Record<string, Record<string, any>>,
    proto = MapCellProto
  ) => {
  ...
  const stockDungeon = (mapKey: Record<string, Record<string, any>>) => {
    Object.keys(mapKey).forEach(key => {
      const subEnvNames = Object.keys(mapKey[key]).filter(keyName =>
        keyName.toLowerCase().includes("env")
      );
