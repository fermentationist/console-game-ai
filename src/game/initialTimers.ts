...
export default [
  ...
  (game: GameType) => {
    if (game.state.turn >= game.timeLimit && !game.state.gameOver) {
      return game.dead(
        "You don't feel so well. It never occurs to you, as you crumple to the ground, losing consciousness for the final time, that you have been poisoned by an odorless, invisible, yet highly toxic gas."
      );
    }
  },
  ...
  // Lightsource timer
  (game: GameType) => {
    game.lightSources.forEach((source: ItemType) =>
      (source as Record<string, any>).decrementCounter()
    );
  },
