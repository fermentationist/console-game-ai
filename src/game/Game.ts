...
export default class Game {
  ...
  spinner = null as InstanceType<typeof LoadingSpinner> | null;
  ...
  setPreference(value: any) {
    ...
    if (this.state.pendingAction === "style") {
      this.log.info(
        `Changing image style from "${this.state.imageStyle}" to "${value}"`
      );
  ...
  setValue(valueOrArrayWithValue: any) {
    const value = Array.isArray(valueOrArrayWithValue)
      ? valueOrArrayWithValue[0]
      : valueOrArrayWithValue;
