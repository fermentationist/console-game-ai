...
const descriptions = {
  preface:
    "You slowly open your eyes. Your eyelids aren't halfway open before the throbbing pain in your head asserts itself. The last thing you can remember is taking your dog for a walk. Your current surroundings look entirely unfamiliar.",
  ...
  intro: (game: GameType) => {
    ...
    game.log.custom(
      "by Dennis Hodges\ncopyright 2019-2023",
      "font-size:100%;color:lightgray;padding:0 1em;"
    );
    game.log.intro(intro_1);
    game.log.codeInline(
      (descriptions as Record<string, any>).introOptions(game)
    );
  },
  ...
  captured: [
    'As you step out onto the front porch, you struggle to see in the bright midday sun, your eyes having adjusted to the dimly lit interior of the house. You hear a surprised voice say, "Hey! How did you get out here?!" You spin around to see the source of the voice, but something blunt and heavy has other plans for you and your still aching skull. You descend back into the darkness of sleep.',
    "Groggily, you lift yourself from the floor, your hands probing the fresh bump on the back of your head.",
  ],
  ...
  help: game => {
    ...
    const italicCodeStyle =
      "font-family:courier;color:#29E616;font-size:115%;font-style:italic;line-height:2;";
    const codeStyle =
      "font-family:courier;color:#29E616;font-size:115%;line-height:1.5;";
    const text = [
    ...
    const text_2 = [
      ...
      "look ",
      'can only be used to "look around", and not to "look at" something. Please instead use ',
      "examine ",
      "or its shortcut ",
      "x ",
      ...
      "and ",
      "down. ",
      "Simply type the direction you want to move. These may be abbreviated as ",
      "n",
      ", ",
      "s",
      ", ",
  ...
  restore: (game, slotList) => {
    ...
    game.log.codeInline([
      "Please choose which slot number (0 through 9) to restore from. To restore, type an underscore, ",
      "_ ",
      "immediately followed by the slot number.",
    ]);
    const exampleSlot = slotList[0] || "_0";
    game.log.codeInline([
      `For example, type `,
      exampleSlot,
      ` to select slot ${exampleSlot.slice(1)}.`,
    ]);
  },

  save: game => {
    game.log.codeInline([
      "Please choose a slot number (0 through 9) to save your game. To save to the selected slot, type an underscore, ",
      "_",
      " immediately followed by the slot number.",
    ]);
  ...
  doorLock:
    "You hear a short metallic scraping sound, ending in a click. It sounds like the front door being locked from the outside.",
} as Record<
  string,
  | string
  | string[]
  | ((gameContext: GameType, value?: any) => void)
  | ((gameContext: GameType, value?: any) => string)
>;
