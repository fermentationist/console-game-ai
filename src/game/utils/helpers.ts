...
export const cases = (...wordArgs: string[]) => {
  // utility function accepts one or multiple string arguments, and returns [all lowercase], [Capitalized first letter], and [ALL CAPS] variations
  let lc, cases;
...
export const cropFloorMap = (floorMap: string[][]) => {
  ...
  const bottom =
    floorMap.length -
    [...floorMap].reverse().findIndex(row => row.some(cell => cell !== "⬛️")) +
    1;
  ...
  const cropped = floorMap
    .slice(top, bottom)
    .map(row => row.slice(left, right));
  return cropped;
};

// timeout defined outside of function so debounce will work across multiple calls (e.g. if you call debounce twice in a row, the second call will cancel the first call's timeout)
let timeout: ReturnType<typeof setTimeout>;
export const debounce = (func: Function, wait: number, context = {}) => {
  return (...args: any[]) => {
