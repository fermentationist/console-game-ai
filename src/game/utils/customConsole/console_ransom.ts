...
export default (message: string) => {
  ...
  const styles = splitText.map(char => {
    ...
    const style = `font-family:${randomFont()};color:rgb(${r}, ${g}, ${b});font-size:${
      3 + Math.random() / 2
    }em;line-height:${
      Math.random() + 0.5
    }em;background-color:rgb(${br}, ${bg}, ${bb});${randomPadding()}${randomOutline(
      border
    )}`;
    return char === " " ? blankStyle : style;
  });
  const spacedText = splitText.join(" ").split("");
  const spacedStyles = styles
    .map(item => [item, `font-size:${1 + Math.random()}em;`])
    .flat()
    .slice(0, -1);
...
const getColors = () => {
  ...
  const foreground = isGray
    ? randomRGBGrayValues(0, 64)
    : randomRGBValues([64, 64, 64], 128);
  const background = isGray
    ? randomRGBGrayValues(192, 255)
    : randomRGBValues([255, 255, 255], 75);
  const border = isGray
    ? randomRGBGrayValues(0, 128)
    : randomRGBValues([128, 128, 128], 128);
  return { foreground, background, border };
};

const randomPadding = (max = 3, min = 1) =>
  `padding: ${Math.random() * max + min}px ${Math.random() * max + min}px ${
    Math.random() * max + min
  }px ${Math.random() * max + min}px;`;

const randomRGBValues = ([r, g, b] = [128, 128, 128], maxVariance = 25) => {
  const randomlyVary = (baseValue: number, maxVary: number) => {
    const randomAbsoluteVariance = Math.floor(Math.random() * maxVary);
    const randomActualVariance =
      Math.random() >= 0.5
        ? randomAbsoluteVariance * -1
        : randomAbsoluteVariance;
    return Math.max(Math.min(255, baseValue + randomActualVariance), 0);
  };
  return [
    randomlyVary(r, maxVariance),
    randomlyVary(g, maxVariance),
    randomlyVary(b, maxVariance),
  ];
};

const randomRGBGrayValues = (min: number, max: number) => {
  const value = Math.floor(Math.random() * (max - min) + min);
  return [value, value, value];
};

const randomCase = (message: string) => {
...
const randomOutline = (color: number[]) => {
